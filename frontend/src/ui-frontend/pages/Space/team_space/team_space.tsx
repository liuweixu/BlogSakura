...
function App() {
  ...
  const getPictureVOList = async () => {
    const myTeamSpace = await getSpaceVoById({ id: searchParamsSpaceId });
    if (myTeamSpace.data.code === 0 && myTeamSpace.data.data) {
        ...
        <div className="flex justify-end mb-4 gap-4 items-center">
          <span>
            <Button
              type="primary"
              onClick={() =>
                navigate(`/personal_space/team_pictures/${searchParamsSpaceId}`)
              }
              icon={<TeamOutlined />}
            >
              团队空间管理
